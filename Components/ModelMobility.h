 ...
 * 
 * <COMMENTS>
 * This header introduces a mobility system model of an AP and STAs.
 * There are 5 types of mobility:
 *  (1) static
 *  (2) uniform walk (at the same speed and the same direction)
 *  (3) random waypoint
 *  (4) random walk
 *  (5) random across (each STA picks an arbitary direction from a point on the border to another point on the border)
 */
#pragma once
#ifndef __SDN_LAB_MODELMOBILITY_H
    #define __SDN_LAB_MODELMOBILITY_H
    // headers
    #include <string>
    #include <fstream>
    // headers - ns3
    ...
    #define __SDN_LAB_MODELMOBILITY_LOC_DIST_UNIFORM                  0   // all nodes are at the same point
    #define __SDN_LAB_MODELMOBILITY_LOC_DIST_RANDOM                   1   // all nodes are randomly distributed
    // marcors - mobility
    #define __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_STATIC              0   // all nodes are fixed
    #define __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_UNIFORM_WALK        1   // all nodes walk at the same speed and at the same direction
    #define __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_WAYPOINT     2   // random waypoint model
    #define __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_WALK         3   // random walk model
    #define __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_ACROSS    4   // random direction model
    // namespaces
    using namespace ns3;
    ...
    namespace SdnLab{
        class ModelMobility{
            ...
            // types
            unsigned int typeLocDist = __SDN_LAB_MODELMOBILITY_LOC_DIST_RANDOM;
            unsigned int typeMob = __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_STATIC;
            // AP coordinates
            double apX = 0;
            double apY = 0;
            // other settings
             ...
             */
            ModelMobility(ns3::NodeContainer * ap, ns3::NodeContainer * stas, double radius){
                if(!ap){
                    Toolbox::Error err("/Components", "ModelMobility.h", "ModelMobility", "ModelMobility", "The pointer to the NodeContainer (AP) is empty.");
                    err.SetType2IllegalParameters();
                    throw err;
                }
                if(!stas){
                    Toolbox::Error err("/Components", "ModelMobility.h", "ModelMobility", "ModelMobility", "The pointer to the NodeContainer (STA) is empty.");
                    err.SetType2IllegalParameters();
                    throw err;
                }
                if(radius <= 0 ){
                    Toolbox::Error err("/Components", "ModelMobility.h", "ModelMobility", "ModelMobility", "The radius of ModelMobility is not positive.");
                    err.SetType2IllegalParameters();
                    throw err;
                }
                // accept inputs
            ...
            void LocDistType2Uniform(double staUniformLocX, double staUniformLocY){
                this->typeLocDist = __SDN_LAB_MODELMOBILITY_LOC_DIST_UNIFORM;
                this->staUniformLocX = staUniformLocX;
                this->staUniformLocY = staUniformLocY;
            }
            void LocDistType2Random(){
                this->typeLocDist = __SDN_LAB_MODELMOBILITY_LOC_DIST_RANDOM;
            }
            /**
            ...
            void MobilityType2Static(){
                this->typeMob = __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_STATIC;
            }
            void MobilityType2UniformWalk(double speed, double angle){
                this->typeMob = __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_UNIFORM_WALK;
            ...
            void MobilityType2RandomWaypoint(double speedMin, double speedMax, double pauseMin, double pauseMax){
                this->typeMob = __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_WAYPOINT;
            ...
            void MobilityType2RandomWalk(double speedMin, double speedMax, double pauseMin, double pauseMax){
                this->typeMob = __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_WALK;
                this->staSpeedMin = speedMin;
                this->staSpeedMax = speedMax;
                this->staPauseMin = pauseMin;
                this->staPauseMax = pauseMax;
            ...
            void MobilityType2RandomDirection(double speedMin, double speedMax, double pauseMin, double pauseMax){
                this->typeMob = __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_ACROSS;
                this->staSpeedMin = speedMin;
                this->staSpeedMax = speedMax;
                this->staPauseMin = pauseMin;
                this->staPauseMax = pauseMax;
            ...
            void Init(){
                // parameters check
                // parameters check - AP & STAs 
                if(!this->ap){
                    Toolbox::Error err("/Components", "ModelMobility.h", "ModelMobility", "Init", "The pointer to the NodeContainer (AP) is empty.");
                    err.SetType2IllegalParameters();
                    throw err;
                }
                if(!this->stas){
                    Toolbox::Error err("/Components", "ModelMobility.h", "ModelMobility", "Init", "The pointer to the NodeContainer (STA) is empty.");
                    err.SetType2IllegalParameters();
                    throw err;
                }
                ...
                switch(this->typeLocDist){
                    case __SDN_LAB_MODELMOBILITY_LOC_DIST_UNIFORM:
                        ...
                        break;
                    case __SDN_LAB_MODELMOBILITY_LOC_DIST_RANDOM:                        
                        mobSTAs.SetPositionAllocator(posSTAsRandom);
                        break;
                }
                // set STA - mobility
                switch(this->typeMob){
                    case __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_STATIC:
                        mobSTAs.SetMobilityModel("ns3::ConstantPositionMobilityModel");
                        break;
                    case __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_UNIFORM_WALK:
                        mobSTAs.SetMobilityModel("ns3::ConstantVelocityMobilityModel");
                        break;
                    case __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_WAYPOINT:
                        ...
                        break;
                    case __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_WALK:
                        break;
                    case __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_RANDOM_ACROSS:
                        break;
                }
                mobSTAs.Install(*this->stas);
                // set STA - mobility - Constant Velocity Model Initial speed
                if(this->typeMob == __SDN_LAB_MODELMOBILITY_MOBILITY_TYPE_UNIFORM_WALK){
                    for(i = 0; i < this->stas->GetN(); ++i){
                        this->stas->Get(i)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(this->staFixedSpeedX, this->staFixedSpeedY, 0));
                    }
                }
            }
