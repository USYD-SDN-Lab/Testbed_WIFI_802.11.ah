...
In Figure, AP is static at a coordinate, ![latex](http://latex.codecogs.com/gif.latex?(x_0,%20y_0)); STAs are randomly scattered around the AP within a circle at a radius of ***r*** with an arbitrary speed vector. The speed vector for each STA changes for each second: its absolute value is from ***0.75 m/s*** to ***1.5 m/s*** and its direction is from ***0*** to ***2π***. Please note that this vector is limited inside the circle boundary and so is STAs' moving trajectory. Through tracking the trajectory of a STA, $n$, we can retrieve its coordinate,![latex](http://latex.codecogs.com/gif.latex?(x_n,%20y_n)), at a time point. The distance between the STA and the AP is, ![latex](http://latex.codecogs.com/gif.latex?d=\sqrt{(x_0-x_n)^2+(y_0-y_n)^2})
